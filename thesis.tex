...
Our central theoretical contribution is the formalisation of a fundamental tradeoff: algorithms that maximise sequential pruning effectiveness inherently minimise parallel scalability. We demonstrated this empirically (CECI: 64M EPS single-thread, 0.33$\times$ speedup; VF3PDL: 119K EPS single-thread, 28$\times$ speedup) and explained it theoretically through Amdahl's Law analysis showing that CECI's pruning phase consumes 88--100\% of execution time on dense graphs.

This principle has immediate practical consequence: the ``fastest'' algorithm in sequential benchmarks may be the \textit{slowest} on parallel systems. Prior literature, which evaluated algorithms primarily on single-threaded performance, could not reveal this inversion.
