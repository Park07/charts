...
\title{A Comparative Evaluation of Parallel Subgraph Isomorphism Algorithms: Scalability, Graph Topology, and Reproducibility}

\author{William Park\\
\textit{School of Computer Science and Engineering}\\
\textit{University of New South Wales}\\
Sydney, Australia\\
z5308311@unsw.edu.au}
...
\begin{abstract}
This thesis presents a comprehensive empirical evaluation of parallel subgraph isomorphism algorithms, comparing VF3PDL, CECI, and SLF across diverse graph topologies and query configurations. We address three research questions: (1) How do parallel subgraph matching algorithms scale, and what limits their speedup? (2) How do graph density and topology affect the relative performance of traversal-based versus index-based algorithms? (3) Is there a fundamental tradeoff between pruning effectiveness and parallel scalability? Our experiments reveal that VF3PDL achieves up to 78x speedup through task-based parallelism, while CECI exhibits negative scaling due to synchronisation overhead. We identify a density crossover effect where index-based approaches dominate sparse queries and traversal-based methods excel on dense graphs. Additionally, we document significant reproducibility challenges with GraphMini and CECI implementations that limited our evaluation scope.
\end{abstract}

\noindent\textbf{Keywords:} subgraph isomorphism, parallel algorithms, graph matching, performance evaluation, reproducibility
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{2cm}\textbf{[Figure: rq1\_memory\_scaling.png]}\\\textit{Memory consumption vs thread count}\vspace{2cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{2cm}\textbf{[Figure: rq1\_superlinear.png]}\\\textit{Superlinear speedup on 8-vertex queries}\vspace{2cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{2cm}\textbf{[Figure: rq1\_speedup\_curves.png]}\\\textit{Speedup curves for 16-vertex queries}\vspace{2cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{2cm}\textbf{[Figure: rq1\_speedup\_bar.png]}\\\textit{Speedup at 64 threads (bar chart)}\vspace{2cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{2cm}\textbf{[Figure: rq2\_density\_effect.png]}\\\textit{Sparse vs Dense performance comparison}\vspace{2cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{2cm}\textbf{[Figure: rq2\_scale\_effect.png]}\\\textit{Performance vs graph scale}\vspace{2cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{2cm}\textbf{[Figure: rq2\_winner\_heatmap.png]}\\\textit{Winner heatmap by dataset and query}\vspace{2cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{2cm}\textbf{[Figure: rq3\_grouped\_bar.png]}\\\textit{1 Thread vs 64 Threads comparison}\vspace{2cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{2cm}\textbf{[Figure: rq3\_ceci\_breakdown.png]}\\\textit{CECI execution time breakdown}\vspace{2cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{2cm}\textbf{[Figure: rq3\_speedup.png]}\\\textit{Speedup comparison with annotations}\vspace{2cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{3cm}\textbf{[Figure: graphmini\_dense\_1.png]}\\\textit{GraphMini dense query results (1)}\vspace{3cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{3cm}\textbf{[Figure: graphmini\_dense\_2.png]}\\\textit{GraphMini dense query results (2)}\vspace{3cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{3cm}\textbf{[Figure: graphmini\_patterns.png]}\\\textit{GraphMini pattern limitations}\vspace{3cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{3cm}\textbf{[Figure: graphmini\_reproducibility\_1.png / graphmini\_asan.png]}\\\textit{AddressSanitizer heap-buffer-overflow}\vspace{3cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{3cm}\textbf{[Figure: graphmini\_reproducibility\_2.png / graphmini\_lldb.png]}\\\textit{LLDB debugger EXC\_BAD\_ACCESS}\vspace{3cm}}}
...
This thesis presented a comprehensive empirical evaluation of parallel subgraph isomorphism algorithms. Our findings demonstrate that:

\begin{enumerate}
    \item \textbf{Scaling behaviour is algorithm-specific}: VF3PDL achieves up to 78x speedup through task-based parallelism, while CECI exhibits consistent negative scaling due to synchronisation overhead.

    \item \textbf{Graph topology dictates algorithm selection}: A density crossover effect exists where index-based approaches (CECI) dominate sparse and planar graphs, while traversal-based methods (SLF) excel on dense scale-free graphs.

    \item \textbf{A fundamental pruning-scalability tradeoff exists}: Algorithms with sophisticated sequential pruning (CECI) achieve high single-thread throughput but cannot parallelise effectively. Conversely, minimal-pruning approaches (VF3) scale well but start from a lower baseline.

    \item \textbf{Reproducibility remains a significant challenge}: Critical bugs in GraphMini and CECI implementations limited our evaluation scope, highlighting the importance of rigorous validation in systems research.
\end{enumerate}

These results provide practical guidance for algorithm selection based on workload characteristics and available computational resources.
