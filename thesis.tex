...
\usepackage{helvet}  % Times font (IEEE style)
...
\fancyhead[R]{A Survey on Parallel Subgraph Matching}
\fancyfoot[C]{\thepage}
...
%------------------------------------------------------------------------------
\subsection{RQ1 Validation: Algorithm Performance Comparison}
\label{sec:synthetic-rq1}
%------------------------------------------------------------------------------

Figure~\ref{fig:synthetic-rq1} presents the average throughput (EPS) across all synthetic graph configurations, providing a direct comparison of algorithm performance on controlled inputs.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{figures/synthetic_rq1_comparison.png}
...
The synthetic results reveal a striking performance hierarchy that corroborates the real-world findings:

\begin{itemize}
    \item \textbf{CECI}: 29,138,381 EPS (dominant)
    \item \textbf{SLF}: 167,446 EPS (174$\times$ slower than CECI)
    \item \textbf{VF3PDL}: 10,170 EPS (2,865$\times$ slower than CECI)
\end{itemize}

This performance ordering matches the real-world results where CECI consistently achieved the highest throughput. The magnitude of the performance gap is even more pronounced on synthetic graphs, suggesting that the absence of exploitable structure disproportionately impacts traversal-based algorithms.

\paragraph{Validation of RQ1.}
The synthetic experiments confirm that CECI's index-based parallel enumeration achieves superior throughput regardless of graph structure. The consistent $\sim$30M EPS across diverse synthetic configurations demonstrates that CECI's performance is structure-independent, validating our real-world observation that index-based approaches dominate in absolute throughput.

%------------------------------------------------------------------------------
\subsection{RQ2 Validation: Topology and Density Effects}
\label{sec:synthetic-rq2}
%------------------------------------------------------------------------------

To validate RQ2's findings on topology-dependent performance, we compare algorithm behaviour across Erdős-Rényi (structureless) and Barabási-Albert (scale-free) graphs.

\subsubsection{Erdős-Rényi Random Graphs}

Figure~\ref{fig:synthetic-rq2-er} presents algorithm performance on ER graphs with varying edge density using sparse 8-vertex queries.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/synthetic_rq2_er.png}
...
The ER graph results reveal critical insights into topology-dependent behaviour:

\begin{itemize}
    \item \textbf{CECI} maintains consistent throughput of approximately 28--33M EPS across all graph sizes, demonstrating complete independence from graph structure.
    \item \textbf{SLF} exhibits moderate degradation from 274K EPS (er\_250k) to 104K EPS (er\_3m), representing a 2.6$\times$ slowdown as graph density increases.
    \item \textbf{VF3PDL} suffers catastrophic degradation from 4,387 EPS (er\_250k) to just 99 EPS (er\_3m)---a \textbf{44$\times$ performance collapse}.
\end{itemize}
...
\subsubsection{Barabási-Albert Scale-Free Graphs}

Figure~\ref{fig:synthetic-rq2-ba} presents algorithm performance on BA graphs, which possess hub structures due to preferential attachment.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/synthetic_rq2_ba.png}
...
The BA graph results show similar patterns with notable differences:

\begin{itemize}
    \item \textbf{CECI} maintains consistent $\sim$30--35M EPS, again demonstrating structure independence.
    \item \textbf{SLF} achieves slightly higher throughput on BA graphs (143K--301K EPS) compared to ER graphs, suggesting that hub structures provide modest benefits for stream-based filtering.
    \item \textbf{VF3PDL} still degrades significantly (133 $\rightarrow$ 12 EPS), but the degradation is less severe than on ER graphs, indicating that scale-free structure provides some pruning opportunities.
\end{itemize}
...
\paragraph{Validation of RQ2.}
The synthetic experiments validate RQ2's finding that graph topology significantly affects traversal-based algorithm performance. On structureless ER graphs, VF3PDL's pruning heuristics become ineffective, resulting in 44$\times$ degradation. On BA graphs with hub structures, degradation is reduced to 11$\times$, confirming that degree heterogeneity provides exploitable structure for pruning. CECI's consistent performance across both topologies validates that index-based approaches are topology-independent.

%------------------------------------------------------------------------------
\subsection{RQ3 Validation: Pruning vs Parallelism Tradeoff}
\label{sec:synthetic-rq3}
%------------------------------------------------------------------------------

Figure~\ref{fig:synthetic-rq3} directly illustrates the pruning-parallelism tradeoff by comparing CECI (parallel enumeration) against VF3PDL (pruning-based traversal) on structureless ER graphs.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/synthetic_er.png}
...
The synthetic experiments provide definitive evidence for the pruning-parallelism tradeoff:

\begin{itemize}
    \item \textbf{VF3PDL's pruning relies on structure}: On ER graphs lacking community structure, degree heterogeneity, or clustering, VF3PDL's traversal heuristics cannot effectively reduce the search space. The algorithm degenerates to near-exhaustive exploration, achieving only 99 EPS on er\_3m.

    \item \textbf{CECI's parallelism is structure-agnostic}: The index-based parallel enumeration maintains consistent throughput of $\sim$30M EPS regardless of topological properties. This demonstrates that CECI's performance derives from efficient parallel work distribution rather than structural exploitation.

    \item \textbf{Performance gap quantification}: On the largest ER graph (3M edges), CECI achieves 30,556,861 EPS compared to VF3PDL's 99 EPS---a performance ratio of \textbf{308,655:1} (over 300,000$\times$ faster).
\end{itemize}

\paragraph{Comparison with Real-World Results.}
On real-world graphs such as ENRON and DBLP, VF3PDL typically achieves performance within 10--100$\times$ of CECI. The synthetic results reveal that this relatively competitive performance depends entirely on exploitable graph structure. When structure is absent, the performance gap expands to over 300,000$\times$, validating RQ3's conclusion that pruning effectiveness and parallel scalability represent fundamentally different algorithmic strategies with topology-dependent tradeoffs.

\paragraph{Validation of RQ3.}
The synthetic experiments conclusively validate RQ3: there is a fundamental tradeoff between pruning effectiveness and parallel scalability, mediated by graph topology. Algorithms optimised for pruning (VF3PDL) achieve competitive performance only when graph structure provides pruning opportunities. On structureless graphs, parallel enumeration (CECI) dominates by orders of magnitude.
...
Dense 16-vertex queries produced zero matches across nearly all synthetic configurations, indicating that the generated graphs lack sufficient local density to contain such patterns. This is consistent with the theoretical properties of ER and BA models, which produce globally sparse graphs even at high edge counts.
...
RQ3 & Pruning-parallelism tradeoff exists & Confirmed: 300,000$\times$ gap on structureless graphs \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{The tradeoff is fundamental}: The 300,000$\times$ performance gap on structureless graphs represents the extreme case of the pruning-parallelism tradeoff, validating that these represent fundamentally different algorithmic strategies.
\end{enumerate}
...
We identified a fundamental tradeoff between pruning effectiveness and parallel scalability. Algorithms with aggressive pruning (CECI) achieve high single-thread throughput but poor scaling. Algorithms with minimal pruning (VF3PDL) scale well but depend on exploitable graph structure. Synthetic experiments demonstrated a 300,000$\times$ performance gap between CECI and VF3PDL on structureless graphs, validating this tradeoff.
